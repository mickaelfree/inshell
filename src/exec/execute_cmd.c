...
/*   Updated: 2025/08/23 16:55:03 by mickmart         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void execute_child(t_command *cmd, int index, t_pipeline *pipeline, char ***envp)
{
    // Setup pipes pour cet enfant
    if (index > 0)  // Pas le premier
        dup2(pipeline->pipes[index - 1][0], STDIN_FILENO);
    if (index < pipeline->cmd_count - 1)  // Pas le dernier
    ...
    // Redirections (overrides pipes)
    handle_redirections(cmd);
    
    // Exécuter
    if (cmd->args && cmd->args[0])
    {
        int builtin_ret = is_builtin(cmd->args, envp);
        if (builtin_ret != -1)
            exit(builtin_ret);
        execute(cmd->args, *envp);
    }
...
static int fork_all_processes(t_command *cmds, t_pipeline *pipeline, char ***envp)
{
    t_command *cur = cmds;
...
{
    // Gérer redirections pour builtin
    handle_redirections(cmd);
    
    // Exécuter builtin dans le parent
    int builtin_ret = is_builtin(cmd->args, envp);
...
static void wait_all_children(t_pipeline *pipeline)
{
    ...
    {
        int sig = WTERMSIG(last_exit_status);
        g_last_exit_status = 128 + sig;
        if (sig == SIGQUIT)
            write(STDERR_FILENO, "Quit (core dumped)\n", 19);
    }
...
{
    t_pipeline *pipeline = create_pipeline(cmd_count);
    if (!pipeline)
    {
...
void execute_cmd(t_command *cmds, char ***envp)
{
    ...
    // CAS SPÉCIAL : 1 commande builtin → parent
    if (cmd_count == 1 && cmds->args && cmds->args[0])
    {
        int builtin_ret = is_builtin(cmds->args, envp);
        if (builtin_ret != -1)
        {
